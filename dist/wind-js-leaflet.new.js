...
L.CanvasLayer = (L.Layer ? L.Layer : L.Class).extend({
	...
	_onLayerDidResize: function _onLayerDidResize(resizeEvent) {
		if (resizeEvent) {
			if (this.canvas == this._canvas2) {
				this.canvas = this._canvas1;
	...
	addClass: function(class_name) {
		this._canvas1.classList.add(class_name);
		this._canvas2.classList.add(class_name);
	}
  ...
  */
   ...
   */
	...
	var animate = function animate(bounds, field, extent) {
		...
		var _simple_color_style = function(color) {
		...
		var colorStyles = _simple_color_style(params.color);
		if (params.colorStyles) {
			colorStyles = params.colorStyles();
		}
		...
		var g = params.layer.canvas.getContext("2d");
	...
	var shift = function shift(dx, dy) {
		var canvas = params.layer.canvas,
		    w = canvas.width,
		    h = canvas.height,
...
function windTemperatureColorScale() {
	...
	result.indexFor = function (m) {
		...
		return Math.max(0, Math.min(result.length - 1, Math.round((m - MIN_TEMPERATURE_K) / (MAX_TEMPERATURE_K - MIN_TEMPERATURE_K) * (result.length - 1))));
	};
	return result;
};
   ...
   */
		...
		_initWindy: function _initWindy(data) {
			...
			this._windy = new Windy({
				layer: WindJSLeaflet._canvasLayer,
				data: data,
				colorStyles: windTemperatureColorScale
			});

			// prepare context global var, start drawing
			//this._context = this._canvasLayer._canvas.getContext('2d');
			//this._canvasLayer._canvas.classList.add("wind-overlay");
			this._canvasLayer.addClass("wind-overlay");
			this.onDrawLayer(this._canvasLayer);

			this._map.on('dragstart', WindJSLeaflet._windy.stop);
			this._map.on('zoomstart', WindJSLeaflet._windy.stop);
