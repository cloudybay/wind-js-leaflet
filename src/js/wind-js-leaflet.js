...
L.CanvasOverlay = L.Class.extend({
	...
	onAdd: function onAdd(map) {
		this._map = map;
		this._canvas1 = L.DomUtil.create('canvas', 'leaflet-heatmap-layer leaflet-layer-fade');
		this._canvas2 = L.DomUtil.create('canvas', 'leaflet-heatmap-layer leaflet-layer-fade');
		...
		this._curr_canvas = this._canvas1;
	...
	onRemove: function onRemove(map) {
		...
		var this_canvas1 = null;
		var this_canvas2 = null;
	},
	...
	_resize: function _resize(resizeEvent) {
		if (this._curr_canvas == this._canvas2) {
			this._curr_canvas = this._canvas1;
			...
			this._curr_canvas = this._canvas2;
	        this._canvas2.getContext('2d').clearRect(0, 0, 3000, 3000);
			L.DomUtil.removeClass(this._canvas2, 'leaflet-layer-hide');
			L.DomUtil.addClass(this._canvas1, 'leaflet-layer-hide');
		}
		this._curr_canvas.width = resizeEvent.newSize.x;
		this._curr_canvas.height = resizeEvent.newSize.y;
	},
	_reset: function _reset(e) {
		...
		L.DomUtil.setPosition(this._curr_canvas, topLeft);
		this._redraw();
	},
	...
	_animateZoom: function _animateZoom(e) {
		...
		this._curr_canvas.style[L.DomUtil.TRANSFORM] = L.DomUtil.getTranslateString(offset) + ' scale(' + scale + ')';
		//this.canvas2.style[L.DomUtil.TRANSFORM] = L.DomUtil.getTranslateString(offset) + ' scale(' + scale + ')';
	},
	...
	var createWindBuilder = function createWindBuilder(uComp, vComp, temp) {
		...
		return {
			...
			data: function data(i) {
				if (temp) {
					//console.log(vData[i], temp.data[i]);
   ...
   */
	...
	var animate = function animate(bounds, field, extent) {
		...
		function windTemperatureColorScale(minTemp, maxTemp) {
			...
			// lwsu: return only one color
			var result = ["rgb(238,247,217)"];
	...
	var start = function start(bounds, width, height, extent, canvas) {
...
var WindJSHelper = {
	...
	redraw: function redraw(overlay, params) {
		...
		WindJSHelper.timer = setTimeout(function () {
			...
			WindJSHelper.windy.start([[0, 0], [size.x, size.y]], size.x, size.y, [[bounds._southWest.lng, bounds._southWest.lat], [bounds._northEast.lng, bounds._northEast.lat]], params.canvas);
	...
	initWindy: function initWindy(data) {
		...
		//WindJSHelper.map.on('dragend', WindJSHelper.clearWind);
		WindJSHelper.map.on('zoomstart', WindJSHelper.windy.stop);
		//WindJSHelper.map.on('zoomend', WindJSHelper.clearWind);
	...
	destroyWind: function destroyWind() {
		...
		WindJSHelper.canvasOverlay.clearCanvas();
		//WindJSHelper.context = WindJSHelper.canvasOverlay.canvas().getContext('2d');
		//if (WindJSHelper.context) WindJSHelper.context.clearRect(0, 0, 3000, 3000);
...
var WindJSLeaflet = function WindJSLeaflet(options) {
	...
	checkWind(options).then(function () {
		...
		if (options.map) {
			map.addLayer(WindJSHelper.canvasOverlay);
		}
	}).catch(function (err) {
